...
@php
    ...
    $action = $action ?? ($isEdit ? route($route . '.update', $editData->{$primaryKey}) : route($route . '.store'));
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    document.querySelectorAll('.autocomplete-select').forEach(function(el) {
                        ...
                        if (el.id === 'trpPengajuanId') {
                            // Update field tersembunyi saat item dipilih
                            $select.on('select2:select', function(e) {
                                const item = e.params.data;

                                $('#trpAnggotaId').val(item.anggotaId ?? '');
                                $('#trpAnggotaNama').val(item.anggotaNama ?? '');
                                $('#trpNominalPinjaman').val(item.nominalPinjaman ?? '');
                                $('#trpTenor').val(item.tenor ?? '');
                                $('#trpBunga').val(item.bunga ?? '');
                                console.log(item);
                            });
